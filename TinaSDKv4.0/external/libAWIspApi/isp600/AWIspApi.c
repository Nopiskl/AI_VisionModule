...
static int awIspStart(int isp_id)
{
    ...
    if (isp_id < 0 || isp_id >= MAX_ISP_NUM) return -1;
    ret = isp_init(isp_id);
    if (ret != 0) return ret;
    ret = isp_run(isp_id);

    if (ret != 0) {
        /* pthread_create errors are positive; undo the successful isp_init. */
        isp_exit(isp_id);
...
static int awIspStop(int isp_id)
{
    ...
    if (isp_id < 0 || isp_id >= MAX_ISP_NUM) return -1;
    ret = isp_stop(isp_id);
    /* Current isp_exit already joins its worker. */
    int exit_ret = isp_exit(isp_id);
    if (ret == 0) ret = exit_ret;
