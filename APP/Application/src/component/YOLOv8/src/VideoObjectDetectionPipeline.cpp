...
private:
    ...
    void captureFrames()
    {
		...
		cv::Size displaySize(
			static_cast<int>(screen_info.xres),
			static_cast<int>(screen_info.yres));
		...
		if (!ofs.is_open()) {
			throw std::runtime_error("Can't open framebuffer device for writing");
		}
    ...
    void performInference()
    {
        while (!done.load()) {
            ...
            auto results = nnRuntime.run([this](int bufferIndex, void *buffer, NeuralNetworkRuntime::InputDataFormat elementDataFormat, std::size_t){
    ...
    static YoloV8Processor createYoloV8Processor(VideoObjectDetectionPipeline::Config& config) {
        ...
        yoloV8ProcessorConfig.rectConfidenceThreshold =
            config.rectConfidenceThreshold;
