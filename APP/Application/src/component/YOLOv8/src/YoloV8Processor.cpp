...
public:
    ...
    void preProcess(cv::Mat &img)
    {
        ...
        scaleRatio = std::min(static_cast<float>(config.imgSize.height) / imgHeight,
                              static_cast<float>(config.imgSize.width) / imgWidth);
        scaleRatio = std::min(scaleRatio, 1.0f);
    ...
    std::vector<Detection> postProcess(int dataElementType, void *data)
    {
        ...
        for (std::size_t i = 0; i < nmsResult.size(); ++i)
        {
            int idx = nmsResult[i];
