...
static void configMainStream(SampleRtspContext *pContext)
{
    ...
    /* Current SDK no longer exposes the old sensor_type RC parameter. */
...
#ifdef SUPPORT_RTSP_TEST
    unsigned char *stream_buf = (unsigned char *)malloc(MAX_FRAME_BUF_SiZE);
    if (NULL == stream_buf)
    {
        aloge("malloc stream_buf failed, size=%d", MAX_FRAME_BUF_SiZE);
        return NULL;
    }
...
int main(int argc, char *argv[])
{
    ...
    /* Register the normal interactive and supervisor shutdown signals. */
    if (signal(SIGINT, handle_exit) == SIG_ERR)
    {
        aloge("can't catch SIGINT");
    }
    if (signal(SIGTERM, handle_exit) == SIG_ERR)
    {
        aloge("can't catch SIGTERM");
    }
